...
class ModelSelect(Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        self.services.state_manager.set_model(server_id, selected_model_id)
...
class ChangeModelCog(BaseCommand):
    """Cog for changing AI models with modern UI components."""
    ...
    @change_model.error
    async def change_model_error(self, ctx: commands.Context, error: Exception):
        """Handle errors in the change_model command."""
        self.logger.error(f"Error in change_model command: {error}")
        await ctx.send(
...
async def setup(bot: commands.Bot):
    ...
    if services:
        await bot.add_cog(ChangeModelCog(bot, services))
